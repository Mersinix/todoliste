...
export default function App() {
  const [todos, setTodos]=useState([
   ...
   const handleComplete=(i)=>{
     setTodos(todos.map(el=> el.id===i?{...el,isCompleted:!el.isCompleted}:{...el,isCompleted:el.isCompleted}))
   };

   const handleEdit=(i)=>{
    setTodos(todos.map(el=> el.id===i?{...el,editable:!el.editable}:{...el,editable:el.editable}))
  }
  ...
  const handleCancel=(id)=>{
    setTodos(todos.map(el=> el.id===id ? {...el,editable:false} : el))
  }
  return (
    <div className="App">
